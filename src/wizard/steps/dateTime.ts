...
export async function sendDateTimeStep(
  ...
  const embed = new EmbedBuilder()
    .setTitle(tWithLocale(locale, 'wizard.datetime_title'))
    .setDescription(
      `${tWithLocale(locale, 'wizard.step_indicator', {
        current: session.stepNumber,
        total: session.totalSteps,
      })}\n\n${tWithLocale(locale, 'wizard.datetime_description', { timezone: session.timezone })}`
    )
    .setColor(EMBED_COLOR_OPEN);

  if (session.data.dateTime) {
    embed.addFields({ name: 'Actuel', value: discordTimestamp(session.data.dateTime, 'F') });
  }

  const buttonRow = new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder()
      .setCustomId(`${CUSTOM_ID.WIZARD_MODAL}:datetime`)
      .setLabel(tWithLocale(locale, 'wizard.datetime_label'))
...
export async function handleDateTimeInteraction(
  ...
  if (interaction.isButton() && interaction.customId === `${CUSTOM_ID.WIZARD_MODAL}:datetime`) {
    const modal = new ModalBuilder()
      .setCustomId(`${CUSTOM_ID.WIZARD_MODAL}:datetime_submit`)
      .setTitle(tWithLocale(locale, 'wizard.datetime_title'));

    const input = new TextInputBuilder()
      .setCustomId('datetime_input')
      .setLabel(tWithLocale(locale, 'wizard.datetime_label'))
      .setPlaceholder(tWithLocale(locale, 'wizard.datetime_placeholder'))
  ...
  if (interaction.isModalSubmit() && interaction.customId === `${CUSTOM_ID.WIZARD_MODAL}:datetime_submit`) {
    const dateStr = interaction.fields.getTextInputValue('datetime_input');
    ...
    session.updateData({ dateTime: parsed });
